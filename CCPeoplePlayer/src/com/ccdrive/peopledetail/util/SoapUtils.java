...
public class SoapUtils {
	...
	public void log(final String type, final HashMap<String, Object> args) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				Iterator<Entry<String, Object>> iter = args.entrySet()
						.iterator();
				while (iter.hasNext()) {
	...
	public void getPeopleClick(final long params) {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String subURL = "ActorbaseStatic";
				try {
					SoapObject request = new SoapObject(NAMESPACE, methodName);
					request.addProperty("in0", params);
					SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(
							SoapEnvelope.VER11);
					envelope.bodyOut = request;
					HttpTransportSE transport = new HttpTransportSE(urlRoot
